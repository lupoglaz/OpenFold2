...
__global__ void fastfold_softmax_fp16(at::Half *input, at::Half *output, int rows,
                                       int cols) {
    int threadidx_x = threadIdx.x / 32;
    int threadidx_y = threadIdx.x % 32;
    int row_offset = blockIdx.x * 4 + threadidx_x;
    ...
    #pragma unroll
        for (int i = 0; i < cols_this_thread; ++i) {
            row_output[lane_id * cols_per_thread + i] =
                static_cast<at::Half>(__fdividef(buf[i], warp_sum));
        }
    }
}
...
__global__ void fastfold_softmax_grad_fp16(at::Half *d_output, at::Half *output,
                                            at::Half *d_input, int rows, int cols) {
    int threadidx_x = threadIdx.x / 32;
    int threadidx_y = threadIdx.x % 32;
    int row_offset = blockIdx.x * 4 + threadidx_x;
    int cols_per_thread = (cols + 31) / 32;
    ...
    if (row_offset < rows) {
        at::Half *row_d_output = d_output + row_offset * cols;
        at::Half *row_output = output + row_offset * cols;
    ...
    #pragma unroll
        for (int i = 0; i < cols_this_thread; ++i) {
            row_d_input[lane_id * cols_per_thread + i] =
                static_cast<at::Half>((dy_buf[i] - warp_sum) * y_buf[i]);
        }
    }
}
...
at::Tensor softmax(at::Tensor input, int rows, int cols) {
    ...
    if (input.dtype() == torch::kFloat32) {
		...
		fastfold_softmax_fp16<<<grid, block>>>((at::Half *)input.data_ptr(),
...
at::Tensor softmax_gradient(at::Tensor d_output, at::Tensor output, int rows, int cols) {
    ...
    if (output.dtype() == torch::kFloat32) {
		...
		fastfold_softmax_grad_fp16<<<grid, block>>>(
...
__global__ void fastfold_softmax_scale_mask_fp16(at::Half *input, at::Half *mask,
                                                  at::Half *output, int rows, int cols,
                                                  float scale, int head) {
    int threadidx_x = threadIdx.x / 32;
    int threadidx_y = threadIdx.x % 32;
    int row_offset = blockIdx.x * 4 + threadidx_x;
...
at::Tensor fused_scale_mask_softmax_forward(at::Tensor input, at::Tensor mask, int rows, int cols,
    ...
    if (input.dtype() == torch::kFloat32) {
		...
		fastfold_softmax_scale_mask_fp16<<<grid, block>>>(
...
__global__ void fastfold_softmax_scale_mask_grad_fp16(at::Half *d_output, at::Half *output,
                                                       at::Half *d_input, at::Half *mask,
                                                       int rows, int cols, float scale, int head) {
    int threadidx_x = threadIdx.x / 32;
    int threadidx_y = threadIdx.x % 32;
    int row_offset = blockIdx.x * 4 + threadidx_x;
    ...
    if (row_offset < rows) {
        ...
        for (int i = 0; i < cols_this_thread; ++i) {
            if (mask_ptr[lane_id * cols_per_thread + i] != 0) {
                row_d_input[lane_id * cols_per_thread + i] =
                    static_cast<at::Half>(scale * ((dy_buf[i] - warp_sum) * y_buf[i]));
            } else {
                row_d_input = 0;
            }
        }
...
at::Tensor fused_scale_mask_softmax_backward(at::Tensor d_output, at::Tensor output,
    ...
    if (output.dtype() == torch::kFloat32) {
		...
		fastfold_softmax_scale_mask_grad_fp16<<<grid, block>>>(
            (at::Half *)d_output.data_ptr(), (at::Half *)output.data_ptr(),
            (at::Half *)grad_input.data_ptr(), (at::Half *)mask.data_ptr(), rows, cols,
            scale, head);
	} else {
